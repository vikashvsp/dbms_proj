...
app = Flask(__name__)
engine = create_engine("mysql://root:root123@localhost/arogya")
...
def book():
    ...
    db.execute("INSERT INTO patient(hid, problem, adhno) VALUES (:hid, :problem, :adhno)",{ "hid": hid, "problem": prob, "adhno": ad})
    db.commit()
...
def adminsuccess():
    ...
    cases=db.execute("SELECT * FROM cases").fetchall()
    if usrname=="vikash123" and paswd=="12345":
